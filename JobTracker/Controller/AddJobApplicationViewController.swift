...
class AddJobApplicationViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIGestureRecognizerDelegate {
    ...
    @IBAction func btnClearFieldsClicked(_ sender: Any) {
        ...
        confirmation.addAction(UIAlertAction(title: "Ok", style: .default, handler: { (_: UIAlertAction!) in self.clearJobApplicationField()
        }))

        confirmation.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: { (_: UIAlertAction!) in
